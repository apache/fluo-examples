...
public class Init extends Configured implements Tool {
    ...
    private static final String TRIE_STOP_LEVEL_PROP  = FluoConfiguration.FLUO_PREFIX + ".stress.trie.stopLevel";
    private static final String TRIE_NODE_SIZE_PROP   = FluoConfiguration.FLUO_PREFIX + ".stress.trie.node.size";

    private static class UniqueReducer
    ...
    static class InitMapper extends Mapper<LongWritable, NullWritable, Text, LongWritable> {
    ...
    static class InitCombiner extends Reducer<Text, LongWritable, Text, LongWritable> {
    ...
    static class InitReducer extends Reducer<Text, LongWritable, Key, Value> {
    ...
    public int run(String[] args) throws Exception {
        if (args.length != 4) {
            log.info("Usage: " + this.getClass().getSimpleName()  + "  <fluo conn props> <app name> <input dir> <tmp dir>");
            System.exit(-1);
        }
